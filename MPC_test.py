...
def mpc_controller(initial_state, initial_force,setpoint, control_horizon, prediction_horizon, state_bounds, bnd):
    ...
    force=np.array(initial_force).reshape(1,prediction_horizon)
    # Define the MPC loop
    #force = deque(np.array(initial_force),maxlen=10)
    #force=np.zeros(10)
    state_buffer = deque(np.array(initial_state),maxlen=10)

        # Define the optimization problem
    def optimization_problem(control):
        x_pred=np.zeros((4,prediction_horizon+1))
        ...
        for i in range(prediction_horizon):
            x_pred[:,i+1]=env.test_step(x_pred[:,i],control[i])
        for j in range(prediction_horizon):
            cost += cost_function(x_pred[:,j+1], control[j])

        return cost

    def constraint_function(u):
        return 1 - np.linalg.norm(u, ord=np.inf)
    bound=tuple((-30,30) for _ in range(prediction_horizon))
...
force_deq=deque([np.zeros_like(force) for _ in range(prediction_horizon)],maxlen=prediction_horizon)
